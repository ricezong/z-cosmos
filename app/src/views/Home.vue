<template>
  ...
  <div class="star-login" @click="router.push('/about')" title="关于">
...
function createAtmosphere(radius, hexColor, opacity = 0.15) {
  ...
  const mainAtmo = new THREE.Mesh(new THREE.SphereGeometry(radius, 48, 48), new THREE.MeshBasicMaterial({
    color: hexColor,
    transparent: true,
    opacity,
    side: THREE.BackSide
  ...
  group.add(mainAtmo)
  const outerAtmo = new THREE.Mesh(new THREE.SphereGeometry(radius * 1.06, 32, 32), new THREE.MeshBasicMaterial({
    color: hexColor,
    transparent: true,
...
function handleBoundary(p) {
  ...
  ;['x', 'y', 'z'].forEach(axis => {
    if (p.position[axis] > boundary[axis] - r) {
      p.position[axis] = boundary[axis] - r;
      vel[axis] *= -0.5
    } else if (p.position[axis] < -boundary[axis] + r) {
      p.position[axis] = -boundary[axis] + r;
      vel[axis] *= -0.5
    }
  })
...
function init() {
  ...
  const planet1 = new THREE.Mesh(new THREE.SphereGeometry(1.6, 64, 64), new THREE.MeshStandardMaterial({
    map: tex1,
    roughness: 0.7,
    metalness: 0.05
  }))
  ...
  const planet3 = new THREE.Mesh(new THREE.SphereGeometry(2.0, 64, 64), new THREE.MeshStandardMaterial({
    map: tex3,
    roughness: 0.6,
    metalness: 0
  }))
  ...
  const planet4 = new THREE.Mesh(new THREE.SphereGeometry(1.5, 64, 64), new THREE.MeshStandardMaterial({
    map: tex4,
    roughness: 0.35,
    metalness: 0.12
  }))
  ...
  const ringInner = new THREE.Mesh(new THREE.TorusGeometry(2.1, 0.08, 16, 200), new THREE.MeshStandardMaterial({
    color: 0xd8e8f5,
    transparent: true,
    opacity: 0.8,
    side: THREE.DoubleSide
  }))
  const ringOuter = new THREE.Mesh(new THREE.TorusGeometry(2.45, 0.12, 16, 200), new THREE.MeshStandardMaterial({
    color: 0xc0d5ea,
    transparent: true,
    opacity: 0.55,
    side: THREE.DoubleSide
  ...
  const planet5 = new THREE.Mesh(new THREE.SphereGeometry(1.3, 64, 64), new THREE.MeshStandardMaterial({
    map: tex5,
    roughness: 0.45,
    metalness: 0.15,
    emissive: 0x3a1e5a,
...
function animate() {
  ...
  handleCollisions()
  planets.forEach(p => {
    p.rotation.y += p.userData.spin || 0.005
  })
...
.star-login:hover .portal-ring.outer {
  ...
  border-color: rgba(197, 213, 234, 0.95);
  box-shadow: 0 0 28px rgba(168, 188, 212, 0.7), inset 0 0 18px rgba(168, 188, 212, 0.3);
}

.star-login:hover .portal-core {
  box-shadow: 0 0 12px #ffffff, 0 0 28px rgba(197, 213, 234, 1), 0 0 50px rgba(168, 188, 212, 0.8), 0 0 70px rgba(140, 170, 210, 0.4);
}

.star-login:hover .portal-glow {
  inset: -25px;
  background: radial-gradient(circle, rgba(168, 188, 212, 0.25) 0%, rgba(140, 170, 210, 0.15) 40%, transparent 70%);
}
...
.cosmic-footer:hover .footer-content {
  border-color: rgba(120, 190, 255, 0.35);
  box-shadow: 0 6px 28px rgba(40, 100, 180, 0.25), inset 0 1px 0 rgba(255, 255, 255, 0.12);
}
...
.icp-link:hover {
  box-shadow: 0 0 16px rgba(80, 160, 255, 0.35);
}

.police-link:hover {
  box-shadow: 0 0 16px rgba(100, 200, 160, 0.35);
}

.footer-divider {
  width: 1px;
  height: 14px;
  background: linear-gradient(to bottom, transparent, rgba(100, 180, 255, 0.4), transparent);
  opacity: 0.6;
}
