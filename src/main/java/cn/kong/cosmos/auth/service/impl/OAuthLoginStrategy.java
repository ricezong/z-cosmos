...
public class OAuthLoginStrategy implements LoginStrategy<Map<String, String>> {
    ...
    public AuthTokenResp executeLogin(Map<String, String> params) {
        ...
        // 6. 签发双 Token
        return tokenService.issueTokens(user.getUserId(), user.getNickname());
    }
    ...
    private String getOAuthAccessToken(OAuthProvider provider, String code) {
        ...
        return switch (provider) {
            case GITHUB -> getGithubAccessToken(config, code);
            case GITEE -> getGiteeAccessToken(config, code);
            case QQ -> getQQAccessToken(config, code);
            default -> throw new IllegalArgumentException("不支持的 OAuth 提供商：" + provider.getId());
        };
    }
