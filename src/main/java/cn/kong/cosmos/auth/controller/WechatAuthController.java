 ...
 * 5. 前端轮询到 SUCCESS 后获取 Access Token
 */
@Slf4j
@RestController
    ...
    public Result<Map<String, Object>> checkLoginStatus(@RequestBody WechatLoginStatusReq req,
        ...
        if ("SUCCESS".equals(status)) {
            ...
            if (openId != null) {
                ...
                cookieTokenHelper.setRefreshTokenCookie(response, tokenResp.getRefreshToken());
                // Access Token 在响应体返回，前端存内存
    ...
    public String handleWechatCallback(HttpServletRequest request) {
        ...
        try {
            if ((msgType != null ? msgType : "").equals("event")) {
                handleEvent(event, inMessage);
            } else {
                log.debug("忽略非事件消息：msgType={}", msgType);
            }
        } catch (Exception e) {
