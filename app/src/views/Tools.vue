    ...
    <div class="tool-panel" v-show="activeTool === 'timer'">
      <div class="tool-box">
        ...
        <div class="timer-btns">
          ...
          <button class="add-note-btn" style="background:rgba(144,166,196,0.2)" @click="pauseTimer"><i class="ri-pause-line"></i> 暂停</button>
...
function startTimer() {
  ...
  timerId = setInterval(() => { timerSeconds.value--; if (timerSeconds.value <= 0) { clearInterval(timerId); timerRunning.value = false; timerId = null; toast.info('时间到！') } }, 1000)
}
...
function copyText() { navigator.clipboard.writeText(textInput.value).then(() => toast.success('已复制到剪贴板')) }
...
.tool-card { border: 1px solid rgba(144,166,196,0.2); border-radius: 16px; padding: 20px 15px; text-align: center; cursor: pointer; transition: 0.3s; }
.tool-card:hover, .tool-card.active { border-color: rgba(144,166,196,0.5); box-shadow: 0 5px 20px rgba(144,166,196,0.15); transform: translateY(-3px); }
.tool-card .icon { font-size: 2rem; margin-bottom: 10px; }
.tool-card .name { font-size: 0.9rem; color: #c5d5ea; }
.tool-panel { display: block; }
.tool-box { border: 1px solid rgba(144,166,196,0.25); border-radius: 20px; padding: 25px; backdrop-filter: blur(10px); }
.tool-box h3 { font-weight: 400; margin-bottom: 20px; color: #e8eef7; letter-spacing: 2px; font-size: 1.2rem; }
.note-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(200px, 1fr)); gap: 15px; }
.note-item { background: rgba(144,166,196,0.15); border: 1px solid rgba(144,166,196,0.3); border-radius: 14px; padding: 15px; position: relative; }
.note-item textarea { width: 100%; min-height: 100px; background: transparent; border: none; color: #e8eef7; font-family: inherit; resize: vertical; outline: none; font-size: 0.9rem; line-height: 1.5; }
...
.add-note-btn { padding: 12px 28px; border-radius: 30px; border: none; margin-top: 20px; background: linear-gradient(135deg, #7890b5, #a8bcd4); color: white; cursor: pointer; font-size: 0.9rem; transition: 0.3s; }
.add-note-btn:hover { box-shadow: 0 0 20px rgba(144,166,196,0.4); }
.draw-canvas { background: #0a1520; border-radius: 12px; border: 2px solid rgba(144,166,196,0.3); cursor: crosshair; display: block; margin-bottom: 15px; }
...
.draw-tools button { padding: 8px 18px; border-radius: 20px; border: 1px solid rgba(144,166,196,0.3); background: rgba(144,166,196,0.15); color: #c5d5ea; cursor: pointer; transition: 0.3s; }
.draw-tools button:hover { background: rgba(144,166,196,0.3); }
.calc-display { background: rgba(5,15,25,0.7); border: 1px solid rgba(144,166,196,0.3); border-radius: 16px; padding: 20px; margin-bottom: 15px; text-align: right; font-size: 1.8rem; color: #e8eef7; min-height: 70px; word-break: break-all; }
...
.calc-btn { padding: 18px; border-radius: 14px; border: none; background: rgba(144,166,196,0.15); color: #e8eef7; font-size: 1.1rem; cursor: pointer; transition: 0.2s; }
.calc-btn:hover { background: rgba(144,166,196,0.3); }
.calc-btn.op { background: rgba(144,166,196,0.25); color: #c5d5ea; }
.calc-btn.op:hover { background: rgba(144,166,196,0.4); }
.calc-btn.eq { background: linear-gradient(135deg, #7890b5, #a8bcd4); color: white; grid-column: span 2; }
.calc-btn.eq:hover { box-shadow: 0 0 15px rgba(144,166,196,0.4); }
.calc-btn.clear { background: rgba(192,80,80,0.2); color: #ffaaaa; }
.calc-btn.clear:hover { background: rgba(192,80,80,0.35); }
.timer-display { text-align: center; font-size: 4rem; font-weight: 300; color: #e8eef7; margin: 30px 0; letter-spacing: 4px; font-family: 'Courier New', monospace; }
...
.timer-inputs input { width: 70px; padding: 12px; border-radius: 12px; background: rgba(5,15,25,0.6); border: 1px solid rgba(144,166,196,0.3); color: #e8eef7; font-size: 1.2rem; text-align: center; outline: none; }
.color-preview { width: 100%; height: 120px; border-radius: 16px; background: #8cc0cc; margin-bottom: 20px; border: 2px solid rgba(144,166,196,0.3); display: flex; align-items: center; justify-content: center; font-size: 1.2rem; font-weight: bold; letter-spacing: 1px; }
...
.text-area-box { width: 100%; min-height: 180px; padding: 15px; border-radius: 14px; background: rgba(5,15,25,0.6); border: 1px solid rgba(144,166,196,0.3); color: #e8eef7; font-family: inherit; font-size: 0.95rem; outline: none; resize: vertical; margin-bottom: 15px; }
.text-stats { display: flex; gap: 20px; flex-wrap: wrap; margin-bottom: 20px; font-size: 0.9rem; }
.text-stat-item { background: rgba(144,166,196,0.15); padding: 8px 16px; border-radius: 12px; border: 1px solid rgba(144,166,196,0.2); }
.text-actions { display: flex; gap: 10px; flex-wrap: wrap; margin-bottom: 15px; }
.text-actions button { padding: 10px 20px; border-radius: 30px; border: 1px solid rgba(144,166,196,0.3); background: rgba(144,166,196,0.15); color: #c5d5ea; cursor: pointer; transition: 0.3s; font-size: 0.85rem; }
.text-actions button:hover { background: rgba(144,166,196,0.3); }
.empty-state { text-align: center; padding: 50px 20px; opacity: 0.5; }
