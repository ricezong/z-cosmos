...
public class UserQueryServiceImpl implements UserQueryService {
    ...
    private User queryByProviderAndOpenId(String provider, String openId) {
        ...
        switch (provider.toLowerCase()) {
            ...
            case "github" -> wrapper.eq(User::getGithubOpenid, openId);
            case "gitee" -> wrapper.eq(User::getGiteeOpenid, openId);
            case "qq" -> wrapper.eq(User::getQqOpenid, openId);
            default -> throw new IllegalArgumentException("不支持的 OAuth 提供商：" + provider);
        }
    ...
    private void setProviderOpenId(User user, String provider, String openId) {
        switch (provider.toLowerCase()) {
            ...
            case "github" -> user.setGithubOpenid(openId);
            case "gitee" -> user.setGiteeOpenid(openId);
            case "qq" -> user.setQqOpenid(openId);
            default -> throw new IllegalArgumentException("不支持的 OAuth 提供商：" + provider);
        }
    }
